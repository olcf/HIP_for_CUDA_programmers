...
class Matrix {
  ...
  public:
    ...
    Matrix(size_t num_row, size_t num_col)
      ...
      SAFE_CALL_CUDA(cudaMemset((void*)h_, 0, num_elt_up_ * sizeP));
#else
...
void set_input_matrix(Matrix_t& a, size_t base_vector_num,
  ...
  set_input_matrix_kernel<Matrix_t> <<<
    ...
    0, stream >>> (a, a.nr(), a.nc(), a.nru(), a.d(), base_vector_num, value);
#else
...
void perform_gemm(cublasHandle_t accelblas_handle, size_t m, size_t n, size_t k,
  ...
  if (compute_capability() >= 700) {
    ...
    ASSERT(status == CUBLAS_STATUS_SUCCESS && "Failure in call to cublasGemmEx.");
