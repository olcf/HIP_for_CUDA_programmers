...
int main(const int argc, const char** argv) {
  ...
  float *d_buf;
  cudaErrorCheck( cudaMalloc(&d_buf, bytes) );
  Body *d_p = (Body*)d_buf;
  ...
  for (int iter = 1; iter <= nIters; iter++) {
    ...
    cudaErrorCheck( cudaMemcpy(d_buf, buf, bytes, cudaMemcpyHostToDevice) );
    bodyForce<<<nBlocks, BLOCK_SIZE>>>(d_p, dt, nBodies); // compute interbody forces
    cudaErrorCheck( cudaMemcpy(buf, d_buf, bytes, cudaMemcpyDeviceToHost) );
    ...
    cudaErrorCheck( cudaEventElapsedTime(&time_ms, start, stop) );
